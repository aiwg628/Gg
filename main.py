...
# --- خادم ويب وهمي للاستضافة على Railway / Replit ---
app = Flask('')
...
async def strip_roles(member: discord.Member, reason: str, target_info=None, extra_info=None):
    ...
    # جلب الرتب التي يمكن للبوت إزالتها
...
@bot.event
async def on_ready():
    print(f"✅ تم تسجيل الدخول بنجاح باسم: {bot.user.name} ({bot.user.id})")
...
async def on_message(message):
    ...
    # 1. نظام الويب هوك (Webhook) - ينحذف وينحذف كل رسايله إذا منشن رول أو إيفري ون/هير
    if message.webhook_id:
        has_role_mention = len(message.role_mentions) > 0
        has_everyone_here = message.mention_everyone or "@everyone" in message.content or "@here" in message.content
        
        if has_role_mention or has_everyone_here:
            try:
                await message.delete()
                
                # حذف جميع رسائل الويب هوك نفسه في الروم
                def check_wh(m):
                    return m.webhook_id == message.webhook_id
                
                await message.channel.purge(limit=100, check=check_wh)
                
                # إزالة الويب هوك تماماً
                try:
                    webhook = await bot.fetch_webhook(message.webhook_id)
                    await webhook.delete(reason="حذف تلقائي - نشر منشنات غير مصرح بها")
                except Exception:
                    pass
                    
                await send_owner_embed(
                    "🚨 حذف وتنظيف ويب هوك مخالف",
                    f"تم مسح رسائل وحذف الويب هوك بسبب عمل منشن في الروم {message.channel.mention}",
    ...
    # مستثنى: الأونرات، أو البوتات المستثناة
    if message.author.id in TRUSTED_IDS or message.author.id in EXEMPTED_BOTS:
        await bot.process_commands(message)
        return

    # 2. حماية روابط الدعوات
    ...
    # 3. حماية البوتات (إذا قام بوت غير مستثنى بمنشن @everyone أو @here مرتين في دقيقتين)
    if message.author.bot:
        if has_everyone_here:
            if check_spam_action(message.author.id, "bot_everyone_spam", max_count=2, seconds=120):
                try:
                    await message.delete()
                    await message.guild.ban(message.author, reason="[الحماية] - منشن everyone/here مرتين خلال دقيقتين")
                    await send_owner_embed(
                        "🚨 تبنيد بوت مخالف",
                        f"🤖 **البوت:** {message.author.mention}\n🆔 **الايدي:** `{message.author.id}`\n❓ **السبب:** منشن @everyone أو @here مرتين خلال دقيقتين"
                    )
                except Exception:
                    pass
                return
    ...
    # 4. حماية الأعضاء العاديين (الأشخاص العاديين لا تتأثر رتبهم بالمنشن العادي، بل عند 3 منشنات رول أو إيفري ون/هير في 3 دقائق)
    if has_everyone_here or has_role_mentions:
        if check_spam_action(message.author.id, "user_broad_mentions", max_count=3, seconds=180):
            try:
                await message.delete()
            except Exception:
                pass
            await strip_roles(message.author, "منشن متكرر للرولات أو الإيفري ون (3 منشنات خلال 3 دقائق)")
            return
